...
public class ScriptInterpreter {
  ...
  public static class Instance {
    ...
    public Instance(final Token token) {
      this._type = Type.UNDEFINED;
      this._token = token;
    }

    public Instance(final Type type, final Object value) {
      this._type = type;
      this._value = value;
    }

    public Instance(final Type type, final Object value, final Token token) {
      this._type = type;
      this._value = value;
      this._token = token;
    }
    ...
    public Object castTo(final Class<?> clazz) throws CastException {
      ...
      if(n.equals("float") || n.equals("java.lang.Float")) return new Float(this.getRealValue());
      if(n.equals("double") || n.equals("java.lang.Double")) return this.getRealValue();
  ...
  private static class BindProxy implements Binder {
    ...
    public Map<String, Instance> getBinds() {
      ...
      for(final Method method : methods) {
        BindData data = ret.containsKey(method.getName())
            ? (BindData)ret.get(method.getName()).getInternalData()
            : null;
        if(data != null)
        {
        ...
        Instance i = new Instance(Instance.Type.FUNCTION, new Function() {
          ...
          public Instance apply(final Instance callee, final Instance[] params) throws Throwable {
            ...
            for(int i = 0; i < to.length; ++i) to[i] = params[i].castTo(methodParams[i]);
            Object ret = m.invoke(data.that, to);
  ...
  public void execute(final String script, final Map<String, Instance> injects) throws Throwable {
    ...
    if(injects != null) heap.putAll(injects);
